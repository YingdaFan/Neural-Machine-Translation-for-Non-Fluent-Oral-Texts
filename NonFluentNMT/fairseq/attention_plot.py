...
def _main(args, output_file):
    ...
    # Load dataset splits
    task = tasks.setup_task(args) #对于翻译任务来说就是加载字典
    task.load_dataset(args.gen_subset) #加载数据，数字化
    ...
    models, _model_args = checkpoint_utils.load_model_ensemble(
        ...
        arg_overrides=eval(args.model_overrides), #model_overrides是命令行参数中要修改的模型超参数
        task=task,
    ...
    itr = task.get_batch_iterator(
        ...
        max_positions=utils.resolve_max_positions( #取各个位置的最大值的交集
            task.max_positions(),
    ...
    has_target = True
    wps_meter = TimeMeter()
    for sample in progress: #每次循环是一个batch
        ...
        num_generated_tokens = sum(len(h[0]['tokens']) for h in hypos) #h[0]是beam_size个输出中分数最高的
        gen_timer.stop(num_generated_tokens) #记录持续时间，以及共计生成的token数
        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            ...
            for j, hypo in enumerate(hypos[i][:args.nbest]):
                ...
                if not args.quiet:
                    ...
                    #attn_value = hypo['attention']
                    #hypo_ = (hypo_str.strip() + " </s>").split(" ")
                    #src_ = (src_str.strip() + " </s>").split(" ")
                    #plt.rcParams['font.sans-serif'] = ['SimHei']
                    #plot(attn_value, src_, hypo_)
                    #plt.savefig('att_plot/picture_' + str(sample["id"].item()) + '.jpg')
