...
class MultitaskDDTranslationCriterion(LabelSmoothedCrossEntropyCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss = self.compute_sum_of_loss(model, net_output, sample, reduce=reduce)
    ...
    def compute_sum_of_loss(self, model, net_output, sample, reduce=True):
        ...
        loss_all = self.lambda_dd * loss_label + (1-self.lambda_dd) * loss
        nll_loss_all = self.lambda_dd * nll_loss_label + (1-self.lambda_dd) * nll_loss
        return loss_all, nll_loss_all